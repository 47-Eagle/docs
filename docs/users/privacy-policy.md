---
id: privacy-policy
title: Privacy Policy
sidebar_label: Privacy Policy
---

# Privacy Policy

*Last Updated: October 7, 2025*

## 1. Introduction

47 Eagle Finance, Inc. ("47 Eagle Finance," "we," "us," or "our") is committed to protecting the privacy and security of your personal information. This Privacy Policy explains how we collect, use, disclose, and protect your information when you use our Eagle Omnichain Vault Protocol and related services.

By using our services, you consent to the collection, use, and disclosure of your information as described in this Privacy Policy.

## 2. Information We Collect

### Personal Information
We may collect the following types of personal information:

**Identity and Contact Information:**
- Full name and legal name
- Email address and phone number
- Residential and business addresses
- Date of birth and nationality
- Government-issued identification documents

**Financial Information:**
- Bank account and payment information
- Income and net worth verification
- Investment experience and sophistication
- Source of funds documentation
- Tax identification numbers

**Technical Information:**
- Wallet addresses and transaction history
- Device information and IP addresses
- Browser type and operating system
- Usage patterns and interaction data
- Cookies and tracking technologies

### Blockchain Data
As a DeFi protocol, we necessarily interact with public blockchain networks where:
- Transaction data is publicly available
- Wallet addresses may be linked to your identity
- On-chain activity is permanently recorded
- Smart contract interactions are transparent

## 3. How We Collect Information

### Direct Collection
We collect information directly from you when you:
- Complete our onboarding and KYC process
- Connect wallets to our platform
- Submit support requests or communications
- Participate in governance voting
- Use our website and services

### Automatic Collection
We automatically collect information through:
- Cookies and web beacons
- Analytics and tracking tools
- Server logs and error reports
- Smart contract event monitoring
- Third-party integration APIs

### Third-Party Sources
We may obtain information from:
- KYC/AML service providers
- Blockchain analytics companies
- Public records and databases
- Credit reporting agencies (where applicable)
- Regulatory and sanctions databases

## 4. How We Use Your Information

### Primary Uses
We use your information to:

**Provide Services:**
- Process vault deposits and withdrawals
- Execute cross-chain transactions
- Provide customer support and communications
- Maintain and improve our platform

**Compliance and Legal:**
- Verify user identity and eligibility
- Comply with KYC/AML requirements
- Meet regulatory reporting obligations
- Prevent fraud, money laundering, and sanctions violations

**Business Operations:**
- Analyze usage patterns and improve services
- Develop new features and capabilities
- Communicate updates and important information
- Maintain security and prevent unauthorized access

### Legal Bases (GDPR)
For users in applicable jurisdictions, we process information based on:
- **Contractual necessity** for service provision
- **Legal obligations** for compliance requirements
- **Legitimate interests** for business operations and security
- **Consent** where explicitly provided

## 5. Information Sharing and Disclosure

### We May Share Information With:

**Service Providers:**
- KYC/AML verification services
- Cloud infrastructure providers
- Analytics and monitoring services
- Customer support platforms
- Security and fraud prevention tools

**Business Partners:**
- DeFi protocol integrations (Charm Finance, LayerZero)
- Blockchain infrastructure providers
- Third-party auditors and security firms
- Professional service providers (legal, accounting)

**Legal and Regulatory:**
- Law enforcement agencies
- Regulatory authorities
- Tax authorities
- Court orders and legal processes
- Emergency situations involving safety or security

### We Do Not:
- Sell personal information to third parties
- Share information for marketing purposes without consent
- Provide information to unauthorized parties
- Use information beyond stated purposes without notification

## 6. Data Security and Protection

### Security Measures
We implement comprehensive security measures including:

**Technical Safeguards:**
- Encryption of data in transit and at rest
- Multi-factor authentication requirements
- Regular security audits and penetration testing
- Secure development and deployment practices
- Network monitoring and intrusion detection

**Organizational Controls:**
- Employee background checks and training
- Access controls and need-to-know principles
- Incident response and breach procedures
- Regular security awareness training
- Third-party security assessments

### Blockchain Considerations
Users should understand that:
- Blockchain transactions are generally irreversible
- On-chain data is publicly visible
- Private keys must be secured independently
- Smart contract risks cannot be eliminated entirely

## 7. Data Retention

### Retention Periods
We retain personal information for:
- **Active users**: Duration of relationship plus 7 years
- **KYC/AML data**: As required by applicable law (typically 5-7 years)
- **Transaction records**: Permanent retention for blockchain compliance
- **Marketing data**: Until consent withdrawal or 3 years of inactivity

### Secure Deletion
When retention periods expire, we securely delete information using:
- Cryptographic erasure methods
- Multi-pass data wiping
- Physical destruction of storage media
- Certificate of destruction documentation

## 8. International Data Transfers

### Transfer Mechanisms
When transferring data internationally, we ensure adequate protection through:
- Standard Contractual Clauses
- Adequacy decisions by relevant authorities
- Certified privacy frameworks
- Explicit user consent where required

### Data Locations
Your information may be processed in:
- United States (primary operations)
- European Union (compliance operations)
- Other jurisdictions where service providers operate

## 9. Your Privacy Rights

### Access and Control
You have the right to:
- Access your personal information
- Correct inaccurate or incomplete data
- Request deletion where legally permissible
- Object to certain processing activities
- Withdraw consent where applicable

### How to Exercise Rights
To exercise privacy rights:
- **Email**: privacy@47eagle.finance
- **Subject Line**: "Privacy Rights Request"
- **Include**: Identity verification and specific request details
- **Response Time**: 30 days for most requests

### Limitations
Some rights may be limited by:
- Legal and regulatory requirements
- Legitimate business interests
- Technical feasibility
- Blockchain immutability

## 10. Cookies and Tracking

### Types of Cookies
We use:
- **Essential cookies** for basic functionality
- **Analytics cookies** for usage measurement
- **Preference cookies** for user settings
- **Security cookies** for fraud prevention

### Cookie Management
You can manage cookies through:
- Browser settings and preferences
- Opt-out mechanisms where available
- Third-party cookie management tools
- Our cookie preference center

## 11. Third-Party Links and Services

### External Links
Our platform may contain links to third-party websites and services. We are not responsible for their privacy practices and encourage you to review their privacy policies.

### Integrated Services
We integrate with third-party DeFi protocols and services that have their own privacy practices and data handling procedures.

## 12. Children's Privacy

Our services are not intended for individuals under 18 years of age. We do not knowingly collect personal information from minors without appropriate consent mechanisms.

## 13. Changes to This Privacy Policy

### Notification Process
We may update this Privacy Policy to reflect:
- Changes in our practices
- Legal or regulatory requirements
- New features or services
- User feedback and industry standards

### Effective Date
Updated policies become effective 30 days after posting, with notification provided through:
- Website notices
- Email communications
- In-app notifications
- Account dashboards

## 14. Regulatory Compliance

### Applicable Laws
We comply with privacy laws including:
- General Data Protection Regulation (GDPR)
- California Consumer Privacy Act (CCPA)
- New York SHIELD Act
- Other applicable privacy legislation

### Data Protection Officer
For GDPR-related matters, contact our Data Protection Officer:
- **Email**: dpo@47eagle.finance
- **Address**: [Physical Address]

## 15. Contact Information

### Privacy Inquiries
For privacy-related questions or concerns:
- **Email**: privacy@47eagle.finance
- **Website**: https://47eagle.finance/privacy
- **Response Time**: 5 business days for general inquiries

### General Contact
- **Legal Department**: legal@47eagle.finance
- **Customer Support**: support@47eagle.finance
- **Documentation**: https://docs.47eagle.finance

---

**This Privacy Policy is designed to be transparent about our data practices while ensuring compliance with applicable privacy laws and regulations.**
